...
bool TreeModel::moveRows(const QModelIndex &sourceParent, int sourceRow,
    ...
    auto ok = true;
    for (auto i = 0; i < count; ++i) {
        ok &= _tree_file->moveItem(sourceParent.internalId(), sourceRow + i,
                                   destinationParent.internalId(),
                                   destinationRow + i);
    }
...
bool TreeModel::moveItem(const QModelIndex &index, int step) {
    return moveRow(index.parent(), index.row(), index.parent(),
                   index.row() + step);
}

bool TreeModel::promoteItem(const QModelIndex &index) {
    if (index.parent().isValid()) {
        auto grandparent = index.parent().parent();

        return moveRow(index.parent(), index.row(), grandparent,
                       _tree_file->childCount(grandparent.internalId()));
    }
